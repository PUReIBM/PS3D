...
#include "ibm.h"
	...
	integer ( kind = 4 ) b, bn
	integer ( kind = 4 ) c
	integer ( kind = 4 ) in, out
	integer ( kind = 4 ) seed1, seed2, seed3
	real(prcn) :: rand1, rand2, rand3

	logical :: first

	public :: rand1, rand2, rand3, seed1, seed2, seed3, init_rand, gen_rand
...
contains
	...
	subroutine init_rand
		...
		if (I_AM_NODE_ZERO) then
			...
			seed1 = int(rand_tmp*c)
			call random_number(rand_tmp)
			seed2 = int(rand_tmp*c)
			call random_number(rand_tmp)
			seed3 = int(rand_tmp*c)

			write ( *, '(a)' ) ' USING LINEAR CONGRUENTIAL RANDOM NUMBER GENERATORE...'
			write ( *, '(a)' ) '  LCRG PARAMETERS:'
			write ( *, '(a,3i12)' ) '  A = ', a
			write ( *, '(a,3i12)' ) '  B = ', b
			write ( *, '(a,3i12)' ) '  C = ', c
			write ( *, '(a)' ) '  INITIAL VALUES:'
			write ( *, '(a,3i12)' ) '  SEED1 = ', seed1
			write ( *, '(a,3i12)' ) '  SEED2 = ', seed2
			write ( *, '(a,3i12)' ) '  SEED3 = ', seed3
			...
			filename = trim(run_name)//"_seed.d"
			open (unit=1, file=trim(filename), status="replace", action="write")
			write (1,*) seed1
			close(1)
		...
		BROADCAST_INT(seed1,1,node_zero,comm_cart_2d)
		BROADCAST_INT(seed2,1,node_zero,comm_cart_2d)
		BROADCAST_INT(seed3,1,node_zero,comm_cart_2d)

#if PARALLEL
		call lcrg_anbn ( a, b, c, nproc, an, bn )
		do i = 1, myid
			in = seed1
			call lcrg_evaluate ( a, b, c, in, out )
			seed1 = out
	...
	subroutine gen_rand
      ...
      call lcrg_evaluate ( a, b, c, in, out )
		seed1 = out
		rand1 = dble(seed1)/dble(c)

      in = seed2
      call lcrg_evaluate ( a, b, c, in, out )
		seed2 = out
		rand2 = dble(seed2)/dble(c)

      in = seed3
      call lcrg_evaluate ( a, b, c, in, out )
		seed3 = out
		rand3 = dble(seed3)/dble(c)
#else
		in = seed1
		call lcrg_evaluate ( an, bn, c, in, out )
		seed1 = out
		rand1 = dble(seed1)/dble(c)
